...
class Tracker:
    ...
    def prev_detections(self):
        ...
        detections = []
        # Write code here
        for i in self.detection_history[-self.lookup_tail_size:]:
            for j in i:
                detections.append(j)
        ...
        for p in detections:
            dict_det[p[0]] = p
        dict_det = list(dict_det.values())
        return detection_cast(dict_det)
    ...
    def bind_tracklet(self, detections):
        ...
        IOU_list = []
        for i in prev_detections:
            for j in detections:
                IOU_list.append([j[0],i[0],iou_score(j[1:],i[1:])])



        # Step 2: sort IOU list
        IOU_list = sorted(IOU_list, key = lambda x:x[2], reverse = True)
